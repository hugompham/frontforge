...
// All views use adaptive width calculated from m.layout.AdaptiveBox for responsiveness
// Standard pattern: contentWidth := m.layout.AdaptiveBox - 4 (accounting for padding)
...
func (m Model) viewTerminalWarning() string {
	...
	b.WriteString(dimensionsStyle.Render(fmt.Sprintf("Current width: %d characters", m.layout.Width)) + "\n")
...
func (m Model) viewWelcome() string {
	...
	// Use adaptive width
	contentWidth := m.layout.AdaptiveBox - 4
	...
	// Forge furnace static ASCII art
	animation := `
      ▒▒▒▒
    ╔══════════╗
    ║ ▒▒▒▒▒▒▒  ║
...
func (m Model) viewGenerating() string {
	...
	// Static forging visualization - hammer and anvil
	animation := `
       [H]
        |
        |
	...
	if m.anim.CurrentTask != "" {
		task := forgeMutedStyle.Render("Current task: ") +
			forgeInfoStyle.Render(m.anim.CurrentTask) +
			AnimatedDots(m.anim.TickCount, 3)
		b.WriteString(task + "\n\n")
	} else {
...
func (m Model) viewError() string {
	...
	errorBoxWidth := m.layout.AdaptiveBox - 8
	if errorBoxWidth < 40 {
		errorBoxWidth = 40
	}
...
// viewPreflightChecks renders the pre-flight validation checks screen
func (m Model) viewPreflightChecks() string {
	var b strings.Builder
	contentWidth := m.layout.AdaptiveBox - 4
	...
	statusMsg := lipgloss.NewStyle().
		...
		Align(lipgloss.Center).
		Width(contentWidth).
		Render("Validating environment before forging...")
	...
	b.WriteString(lipgloss.NewStyle().
		Width(contentWidth).
		Align(lipgloss.Center).
		Render(spinnerStyle.Render(m.spinner.View())) + "\n\n")

	// If we have results, show them
	if m.preflightResults != nil && len(m.preflightResults.Checks) > 0 {
		divider := lipgloss.NewStyle().
			Foreground(colorAnvilGray).
			Render(strings.Repeat("─", contentWidth))
		b.WriteString(divider + "\n\n")
		...
		for _, check := range m.preflightResults.Checks {
			...
			if check.Passed {
				icon = "✓"
				iconColor = colorTemperedGreen
			} else {
				icon = "✗"
				if check.Fatal {
					iconColor = colorCrackedRed
				} else {
					iconColor = colorFlameYellow
				}
			}
			...
			checkLine := fmt.Sprintf("  %s %s", iconStyle.Render(icon), nameStyle.Render(check.Name))
			b.WriteString(checkLine + "\n")

			// Show message indented
			msgStyle := lipgloss.NewStyle().
				Foreground(colorDraftPencil)
			b.WriteString("    " + msgStyle.Render(check.Message) + "\n")

			// Show suggestion if check failed
			if !check.Passed && check.Suggestion != "" {
				suggestionStyle := lipgloss.NewStyle().
					Foreground(colorAshGray).
					Italic(true)
				b.WriteString("    " + suggestionStyle.Render("→ "+check.Suggestion) + "\n")
			}
		...
		if m.preflightResults.FatalError {
			errorStyle := lipgloss.NewStyle().
				...
				Align(lipgloss.Center).
				Width(contentWidth)
			b.WriteString(errorStyle.Render("❌ Critical checks failed - cannot proceed") + "\n\n")
		...
		} else if m.preflightResults.AllPassed {
			successStyle := lipgloss.NewStyle().
				Foreground(colorTemperedGreen).
				Bold(true).
				Align(lipgloss.Center).
				Width(contentWidth)
			b.WriteString(successStyle.Render("✓ All checks passed - proceeding to forge...") + "\n")
		}
	}
